...
3) Desmarque a "quick setup", pode atrapalhar quando for fazer a configuração das coleções;
...
1) Há duas áreas, "Connect to your application" (a que usaremos) e "Acess your data through tools" (acessar o banco de dados por meio de ferramentas);

2) Clicando em "Drivers", você chega na etapa 2. Nela é mostrado a forma convencional de integrar o mongodb e a base da string de conexão (contendo a conta, cluster e a base para user e password). Essa parte de integração será desconsiderada no momento, pois iremos trabalhar com o Prisma.

3) Habilite que outros endereços de IP acessem seu projeto: > network acess > allow acess from anywhere (ip liberado - 0.0.0.0/0)

<br> 


## 4) 🗂 Browser collections 

- Vamos acessar browser collections para adicionar informações;
- collections > add my own data > Crie um banco + coleção > create;
- Insert document > add field > Define o tipo de valor (`string, boolean, object, array, int32/inte34, double`);


<br>

## 5) 📐 Montando o projeto
...
```
- "-D" = Depenência de desenvolvimento; 
